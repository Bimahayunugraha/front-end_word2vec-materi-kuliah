...
import CorpusListItemByCourse from "./CorpusListItemByCourse";
import { Helmet } from "react-helmet";
import CorpusAPI from "../../../apis/corpus.api";
import Breadcrumbs from "../../Breadcrumbs";
import { Link, useNavigate, useParams } from "react-router-dom";
import { useDebounce } from "use-debounce";
import {} from "../../../utils/globalVariable";
import Pagination from "../../Pagination";
import { useSelector } from "react-redux";

const Initial_Corpus_Lists_By_Course = {
  data: [],
  page: 0,
  status: false,
};

const CorpusListsByCourse = () => {
  const [corpusListsByCourse, setCorpusListsByCourse] = useState(Initial_Corpus_Lists_By_Course);
  ...
  useEffect(() => {
    if (debouncedKeyword) {
      CorpusAPI.searchAllFilesListCorpusByCourse(
        course,
        debouncedKeyword.toLowerCase(),
        10,
        page
      ).then((result) => {
        setCorpusListsByCourse({
          status: true,
      ...
      CorpusAPI.getAllFilesListCorpusByCourse(course, 10, page).then((result) => {
        setCorpusListsByCourse({
          status: true,
          data: result.data.payload,
          page: result.data.meta.page,
  ...
  return (
    <div className="mx-auto md:container">
      <Helmet>
        <title>{`Daftar File Korpus - ${course} - Word2vec Mata Kuliah`}</title>
      ...
      <div className="pt-6 pb-6">
        ...
        <div className="flex items-center bg-secondary-soft-white py-2 px-4">
          <Breadcrumbs className="hidden md:block">
            <Breadcrumbs.Content>
              <Breadcrumbs.Link>
                <button
                  onClick={() => navigate("/filelist/corpus/file")}
              ...
              <Breadcrumbs.Link>
                <p className="ml-1 text-xs font-normal text-neutral-40 md:text-sm">Daftar korpus</p>
                <svg
                  aria-hidden="true"
        ...
        <div className="w-full bg-white shadow-6 md:rounded-b-xl">
          <div>
            {corpusListsByCourse.status ? (
              <div>
                {corpusListsByCourse.data.length > 0 ? (
                  <div>
                    <div className="max-h-80 overflow-y-auto">
                      {corpusListsByCourse.data.map((item) => {
                        return <CorpusListItemByCourse key={item.id} data={item} />;
                      })}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-center py-6 text-xs font-semibold leading-7 text-neutral-80">
                    ...
                    Data file korpus berdasarkan mata kuliah tidak ditemukan
                  </div>
                )}
              </div>
            ) : (
...
export default CorpusListsByCourse;
