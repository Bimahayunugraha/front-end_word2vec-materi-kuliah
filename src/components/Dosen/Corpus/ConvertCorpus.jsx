...
import { convertCorpus } from "../../../stores/features/corpusSlice";
...
const ConvertCorpus = () => {
  ...
  const fileMimeTypes = /\.(doc|docx|pdf)$/;
  ...
  const onDrop = (files) => {
    ...
    if (!uploadedFile.name.match(fileMimeTypes)) {
      setErrors({ ...errors, file: "Tipe file yang diunggah harus .doc, .docx, atau .pdf" });
      return;
    ...
    try {
      if (!errors.file) {
        if (file !== null && values.book_title !== "" && values.course !== "") {
          ...
          dispatch(convertCorpus(formData))
            .then((result) => {
              if (!result.error) {
                navigate(`/download/corpus/file/${result.payload.course}/${result.payload.id}`);
                ...
                Toast.fire({
                  icon: "success",
                  title: "Mata kuliah berhasil dikonversi ke korpus",
                });
  ...
  return (
    <div className="mx-auto md:container">
      {file && !errors.file ? (
        ...
        <div className="text-center">
          <h1 className="text-xl font-bold tracking-wide text-primary-soft-violet md:text-2xl">
            Konversi Korpus Teks
          </h1>
          <p className="text-sm font-normal text-neutral-60 md:text-base">
            Mengubah materi mata kuliah dengan format file pdf atau word ke korpus teks
          </p>
        </div>
      )}
        ...
        <div className="pt-6 pb-6">
          ...
          {!errors.file && file ? (
            <div className="w-full bg-white shadow-2xl md:rounded-xl">
              <div className="bg-secondary-soft-white py-6 lg:block"></div>
              <div className="hidden flex-col bg-white px-10 py-4 shadow-4 md:flex-row md:items-center lg:flex">
              ...
              <div className="bg-secondary-soft-white lg:hidden"></div>
            ...
            <div>
              <div className="converter-wrapper w-full shadow-2xl md:rounded-b-xl">
                <div className="px-20 py-10">
                  <ul className="mb-3 max-w-md list-inside list-disc space-y-1 text-sm text-gray-300">
                    <li>
                      File yang diupload harus berformat{" "}
                      <span className="font-bold text-secondary-yellow">.pdf, .doc</span> atau{" "}
                      <span className="font-bold text-secondary-yellow">.docx</span>
                    </li>
                    ...
                    {({ getRootProps, getInputProps }) => (
                      <div {...getRootProps({ className: "drop-zone" })} ref={dropRef}>
                        ...
                        <div className="flex items-center gap-5 pb-5 text-white">
                          <i className="fa-light fa-file-pdf text-3xl md:text-5xl"></i>
                          <span className="self-center whitespace-nowrap text-base font-normal">
                            atau
                          </span>
                          <i className="fa-light fa-file-word text-3xl md:text-5xl"></i>
                        </div>
