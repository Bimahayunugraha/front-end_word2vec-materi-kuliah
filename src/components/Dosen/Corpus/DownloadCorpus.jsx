...
const Initial_Corpus_Without_Content_File_By_Id = {
  data: [],
  status: false,
};

const DownloadCorpus = () => {
  const [corpusWithoutContentFileById, setCorpusWithoutContentFileById] = useState(
    Initial_Corpus_Without_Content_File_By_Id
  );
  ...
  useEffect(() => {
    CorpusAPI.getFileCorpusByIdWithoutContent(course, id).then((result) => {
      setCorpusWithoutContentFileById({ status: true, data: result.data.payload });
    });
  }, []);
  ...
  const handleDownload = async () => {
    ...
    CorpusAPI.downloadCorpus(id).then((res) => {
      const fileName = corpusWithoutContentFileById.data.out_name;
      ...
      Toast.fire({
        icon: "success",
        title: "File korpus berhasil didownload",
      });
  ...
  return (
    <div className="mx-auto lg:container">
      <Helmet>
        <title>{`Download Corpus - ${corpusWithoutContentFileById.data.out_name} - Word2vec Mata Kuliah`}</title>
      ...
      <div className="text-center">
        <h1 className="text-xl font-bold tracking-wide text-secondary-green md:text-2xl">
          File Berhasil Dikonversi Menjadi Korpus
        </h1>
      ...
      <div className="pt-6 pb-6">
        <div className="shadow-7 md:rounded-xl">
          {corpusWithoutContentFileById.status ? (
            <div className="w-full bg-white px-10 py-4 drop-shadow-md md:rounded-t-xl">
              <div className="mb-3 hidden lg:flex lg:items-center">
                ...
                <span className="ml-2 text-sm text-gray-600">
                  {corpusWithoutContentFileById.data.course} •{" "}
                  {corpusWithoutContentFileById.data.book_title}
                </span>
              </div>
              <div className="hidden flex-col md:flex-row md:items-center lg:flex">
                ...
                <div className="min-w-0 flex-1 flex-col items-center">
                  <span className="text-xs font-medium text-neutral-100-2 md:text-sm">
                    {corpusWithoutContentFileById.data.out_name}
                  </span>
                </div>
                ...
                <div className="mr-4 inline-flex items-center md:mr-10">
                  <p className="font-medium">
                    <span className="text-xs text-neutral-80 md:text-sm">
                      {formatFileSize(corpusWithoutContentFileById.data.out_file_size)} |{" "}
                    </span>
                    <span className="text-xs text-neutral-60 md:text-sm">
                      {corpusWithoutContentFileById.data.conversion_type} |{" "}
                    </span>
                    <span className="text-sm text-secondary-green md:text-base">
                      {corpusWithoutContentFileById.data.out_file_mimetype}
                    </span>
                  </p>
                </div>
          ...
          <div className="converter-wrapper flex justify-center px-10 pt-4 pb-4 md:rounded-b-xl">
            <Link to="/corpus" className={backButton}>
              Konversi lagi ke korpus
              <span className="ml-3">
