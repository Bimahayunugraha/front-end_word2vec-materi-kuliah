...
import CourseMaterialListItemByCourse from "./CourseMaterialListItemByCourse";
import Breadcrumbs from "../../Breadcrumbs";
import Pagination from "../../Pagination";
import Tabs from "../Tabs";
import { useSelector } from "react-redux";

const Initial_Course_Material_Lists_By_Course = {
  data: [],
  page: 0,
  status: false,
};

const CourseMateriListsByCourse = () => {
  const [courseMaterialListsByCourse, setCourseMaterialListsByCourse] = useState(
    Initial_Course_Material_Lists_By_Course
  );
  ...
  useEffect(() => {
    if (debouncedKeyword) {
      CourseMaterialAPI.searchAllFilesListsCourseMaterialByCourse(
        course,
        debouncedKeyword.toLowerCase(),
        10,
        page
      ).then((result) => {
        setCourseMaterialListsByCourse({
          status: true,
      ...
      CourseMaterialAPI.getAllFilesListsCourseMaterialByCourse(course, 10, page).then((result) => {
        setCourseMaterialListsByCourse({
          status: true,
          data: result.data.payload,
          page: result.data.meta.page,
  ...
  return (
    <div className="mx-auto md:container">
      <Helmet>
        <title>{`Daftar File Materi Kuliah - ${course} - Word2vec Mata Kuliah`}</title>
      ...
      <div className="pt-6 pb-6">
        ...
        <div className="flex items-center bg-secondary-soft-white py-2 px-4">
          <Breadcrumbs className="hidden md:block">
            <Breadcrumbs.Content>
              <Breadcrumbs.Link>
                <button
                  onClick={() => navigate("/filelist/course-material/file")}
            ...
            <div className="flex items-center">
              ...
              <div className="relative w-full md:w-48 lg:w-80">
                ...
                <input
                  ...
                  placeholder="Cari data materi kuliah"
                  required
                  value={search_query}
        ...
        <div className="w-full bg-white shadow-6 md:rounded-b-xl">
          <div>
            {courseMaterialListsByCourse.status ? (
              <div>
                {courseMaterialListsByCourse.data.length > 0 ? (
                  <div>
                    <div className="max-h-80 overflow-y-auto">
                      {courseMaterialListsByCourse.data.map((item) => {
                        return <CourseMaterialListItemByCourse key={item.id} data={item} />;
                      })}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-center py-6 text-xs font-semibold leading-7 text-neutral-80">
                    ...
                    Data file materi kuliah berdasarkan mata kuliah tidak ditemukan
                  </div>
                )}
              </div>
            ) : (
...
export default CourseMateriListsByCourse;
